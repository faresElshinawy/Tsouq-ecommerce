                            ...
                            @foreach ($permission as $permission_opt)
                                ...
                                <div class="form-check mt-3 d-inline-block mx-1">
                                            <input class="form-check-input  @error('roles') border-danger @enderror" type="checkbox"
                                                value="{{ $permission_opt->id }}" name='permission[]' id="defaultCheck{{ $permission_opt->id }}"
                                                @foreach (old('roles') ?? $rolePermissions as $role_opt)
                                                        @if ($role_opt == $permission_opt->id)
                                                            {{ 'checked' }}
                                                        @endif
                                                @endforeach >
                                    <label class="form-check-label" for="defaultCheck{{ $permission_opt->id }}"> {{ $permission_opt->name }}</label>
                                </div>
