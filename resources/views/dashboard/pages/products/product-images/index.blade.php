...
@section('content')
    <div>
        <div class="container-xxl flex-grow-1 container-p-y">
            <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">{{ $product->name }} /</span>images</h4>
            <div class="card">
                <div class="m-3 d-flex justify-content-between">
                    <div class="col-3">
                        {{-- <input type="text" class="form-control" id="image-search" placeholder="Search..."> --}}
                    </div>
                    @can('product-image create')
                        <a class="btn btn-primary text-white" href="{{ route('products.product-image.create',['product'=>$product->id]) }}">Add New image</a>
                    @endcan
                </div>
                <div class="table text-nowrap">
                    <div id="search-result">
                        <div class="table-responsive  text-nowrap">
                                            ...
                                            <td>
                                                @if (File::exists(public_path('uploads/products/' . $image->image)))
                                                    <img src="{{ asset('uploads/products/' . $image->image) }}" alt
                                                        class="w-px-200 h-auto rounded-square rounded img-fluid" />
                                                @else
                                            ...
                                            <td>
                                                <div class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu">
                                                        @can('product-image delete')
                                                            <form
                                                                action="{{ route('products.product-image.destroy', ['productImage' => $image->id]) }}"
                                                                method="Post">
                                                                @csrf
                                                                @method('delete')
                            ...
                            @else
                                <p class="text-center text-primary my-3">Product Have No images Yet</p>
                            @endif
                        </div>
                        <div class="m-3">
